...
public class ParticleSystemPreview : ObjectPreview
{
    ...
    private void InitPreview()
    {
        ...
        {
            Shader shader = Shader.Find("Universal Render Pipeline/Unlit");//EditorGUIUtility.LoadRequired("Previews/PreviewPlaneWithShadow.shader") as Shader;
            m_FloorMaterial = new Material(shader);
            for (int i = 0; i < shader.GetPropertyCount(); i++)
            {
                Debug.Log($"Property {i} : Name={shader.GetPropertyName(i)} :<b> Type={shader.GetPropertyType(i)}</b> : ID={shader.GetPropertyNameId(i)}");
            }
            m_FloorMaterial.mainTexture = m_FloorTexture;
            m_FloorMaterial.SetColor("_BaseColor", Color.Lerp(Color.gray, Color.clear, 0.75f));
